...
/*function myfunction1(boo,boo1,boo2){
	$.ajax({ url: 'getsite_mysql2.php',
		type: 'post',
		data:{"content":boo,"time":<?php echo "{$_SESSION['user_id']}";?>,"page5":boo1,"page6":boo2}, 
		dataType:'text',
		success: function(msg)
		{	
			$("#txtHint").html(msg);
...
$("#explore").click(function({$page},{$pageSize}){
	var keyword=$("#search").val();
    $.ajax({url:'explore.php',
	type:'post',
	data:{"key":keyword,"page3":$page,"page4":$pageSize,"time":<?php echo "{$_SESSION['user_id']}";?>},
	dataType:'text',
	success: function(msg)
		{	
			$("#txtHint").html(msg);
...
<nav class="navbar navbar-default">
			...
			<form method="POST" action="explore.php" > 
				<div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <button class="btn btn-primary" type="submit" id="explore">检索</button>
                    </span>
				</div>	
			</form>		
		</div>
 ...
 <div class="container">
		...
		//去数据库取数据
	      ...
	      $result = mysqli_query($conn,"select * from book limit $page,$pageSize");
		//}
		if($pageVal <=3){
			$begin =1;
