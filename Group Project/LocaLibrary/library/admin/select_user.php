...
<body>
  ...
  { 
    ...
    $query=$_SESSION['sou'];
    $res=mysqli_query($conn,"select * from reader  WHERE `card`='{$query}' or `name`='{$query}'");
    $numrows = mysqli_num_rows($res);
    $pageNum = ceil($numrows/$pageSize);
    
    $sql = "SELECT card,name,class FROM reader WHERE `card`='{$query}' or `name`='{$query}'limit $page,$pageSize";
